...
class Game(models.Model):
    ...
    booth = models.ForeignKey(
        Booth, on_delete=models.CASCADE, related_name='games')
    title = models.CharField(max_length=255, unique=True)
    release_date = models.DateField()
    game_iframe_src = models.URLField(max_length=1000)
    genre = models.CharField(
        max_length=255, choices=GENRE_CHOICES, default='Adventure')
    game_description = models.TextField()
    image_url = models.URLField(max_length=1000)
    last_updated = models.DateField(auto_now=True)
    technology = models.CharField(
        max_length=50, choices=TECHNOLOGY_CHOICES, default='HTML5')
...
class GameScreenshot(models.Model):
    ...
    screenshot = models.ImageField(
        _("Screenshot"), upload_to=upload_to, null=True, blank=True)
...
class BoothCustomization(models.Model):
    booth = models.OneToOneField(
        Booth, on_delete=models.CASCADE, related_name='customization')
    theme = models.ForeignKey(
        Theme, on_delete=models.SET_NULL, null=True, blank=True, related_name='booths')
...
class PaymentHistory(models.Model):
    ...
    def __str__(self):
        if self.game:
            return f"Game Payment: {self.game.title}"
        elif self.sponsorship:
            return f"Sponsorship Payment: {self.sponsorship.name}"
        else:
